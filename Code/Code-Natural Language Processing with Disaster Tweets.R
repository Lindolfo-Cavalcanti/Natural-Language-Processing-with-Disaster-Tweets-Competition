...
# Clean data
  ...
  # Remove NA's
      ...
      test_token = test_original$text|> tokens(what = "word",
                                               remove_punct = TRUE,
                                               remove_symbols = TRUE,
                                               remove_numbers = TRUE,
                                               remove_hyphens = TRUE
      ...
      test_token = test_token |> tokens_tolower()
      
      # Remove stopwords
      
      test_token = test_token |> tokens_remove(stopwords("en"))
      
      # Remove emotes
      
      emotes_patern = c("u0089û", "\U0001F600-\U0001F64F", "\U0001F300-\U0001F5FF", "\U0001F680-\U0001F6FF", "\U0001F1E0-\U0001F1FF", "\U00002702-\U000027B0", "\U000024C2-\U0001F251")
      
      test_token = test_token |> tokens_remove(emotes_patern)
      ...
      test_token = test_token |> tokens_replace(pattern = contraction, 
                                                replacement = contraction_replacement, valuetype = "fixed")
      
      
      # Stemming
      
      test_token = test_token |> tokens_wordstem(language = "english")
      
      # Remove URLs
      
      url_pattern = "http"
      
      test_token = test_token |> tokens_remove(pattern = url_pattern)
      
      # Remove whitespace
      
      test_token = test_token |> tokens_remove(pattern = "\\s+")
      
      # Remove u0089û_
      
      test_token = test_token |> tokens_remove(pattern = "u0089û_")
      
      # Remove NA's
      
      test_token = test_token[!is.na(test_token)]
      
      
      # View token
      
      test_dfm = test_token |> dfm(tolower = FALSE)
      
      test_dfm <- test_dfm |> dfm_trim(min_termfreq = 3)
      
      test_matrix = test_dfm |> as.matrix()
      
      View(test_matrix)
      
      test_original$target = NA
  
# View token

      train_dfm = train_token |> dfm(tolower = FALSE)

      train_dfm <- train_dfm |> dfm_trim(min_termfreq = 3)
      
      all_terms <- union(featnames(train_dfm), featnames(test_dfm))
      
      train_dfm <- dfm_select(train_dfm, pattern = all_terms, valuetype = "fixed", selection = "keep")
      test_dfm = dfm_select(test_dfm, pattern = all_terms, valuetype = "fixed", selection = "keep")
...
test_token_df = cbind(
  target = test_original$target, convert(test_dfm, to = "data.frame")
)
...
# Create Model
  ...
  # Build Model
    ...
    rpart_cv_1 = caret::train(target ~ ., data = train_token_df,
...
result = predict(object = rpart_cv_1, newdata = test_token_df)
